...
if __name__ == "__main__":
    ...
    majority_class_arr, neighbors_indices = zip(
        *[k_nearest_neighbors(X, y, i, K) for i in range(X.shape[0])]
    )
    majority_class_arr, neighbors_indices = (
        list(majority_class_arr),
        list(neighbors_indices),
    )
    ...
    ani = FuncAnimation(
        fig, animate, frames=range(X.shape[0]), blit=False, interval=300, repeat=False
    )
    plt.show()
