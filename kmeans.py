...
def k_means(points, K, max_iters=20, tol=1e-4):
    ...
    cluster_ind = np.zeros(N_points, dtype=int) # 0 means the ith point belongs to the cluster 0
    ...
    for iter in range(max_iters):
        ...
        for i in range(N_points):
            ...
            distances = np.array([np.linalg.norm(points[i] - cluster_centers[k]) for k in range(K)])
...
if __name__ == "__main__":
    ...
    print("Initial sum of squared distances:", sum((points[i] - cluster_centers[cluster_ind[i]])** 2 for i in range(N_points))**0.5)
    ...
    for i in range(N_points):
        for k in range(K):
            if points[i] != cluster_centers[cluster_ind[i]]:
                ...
                raise Exception("Cluster assignments are incorrect, please double-check your code.")
            else:
                if i % 100 == 0:
                    print(f"Point {points[i]} is correctly assigned to cluster {cluster_ind[i]} with center mean as {cluster_centers[cluster_ind[i]]}")
