...
def compute_bayes_rule(p_cancer, p_test_positive_given_no_cancer, p_test_positive_given_cancer):
    ...
    p_test_positive = (p_test_positive_given_cancer * p_cancer) + (p_test_positive_given_no_cancer * (1 - p_cancer))
    ...
    p_test_negative = ((1 - p_test_positive_given_cancer) * p_cancer) + ((1 - p_test_positive_given_no_cancer) * (1 - p_cancer))
    ...
    p_cancer_given_test_positive = p_test_positive_given_cancer * p_cancer / p_test_positive
    # p(cancer | test negative) = p(test negative | cancer) * p(cancer) / p(test negative)
    #                           = (1 - p(test positive | cancer)) * p(cancer) / p(test negative)
    p_cancer_given_test_negative = (1 - p_test_positive_given_cancer) * p_cancer / p_test_negative
    return {
